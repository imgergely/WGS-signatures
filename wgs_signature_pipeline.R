...
CFG <- list(
  isomut_snv    = "input/all_SNVs.isomut.gz",
...
# Five prespecified references shown in Figure 2C
prespec_ref <- list(
  "Control"             = c("control"),
...
required_inputs <- c(CFG$isomut_snv, CFG$blacklist_bed, CFG$clone_key_csv)
missing_inputs <- required_inputs[!file.exists(required_inputs)]
if (length(missing_inputs) > 0L) {
  stop("Missing required input file(s): ", paste(missing_inputs, collapse = ", "))
}

# Table S1 is used for raw ID to protocol name mapping
...
# read the IsoMut SNV table and validate the complete panel
...
if (!all(toupper(snv$type) == "SNV")) {
  stop("The input call set contains non-SNV records.")
}
if (!all(snv$ref != snv$mut)) {
  stop("The input call set contains records with identical REF and ALT alleles.")
}
...
# =========================================================================
# STEP 33 — ENCODE blacklist filtering and per-clone mutation counts
# =========================================================================
...
burden <- data.frame(sample = names(grl), n_filtered = lengths(grl),
                     ...
                     row.names = NULL)
# =========================================================================
# STEP 34 — Final cohort validation and Table S3 output
# =========================================================================
...
message(
  "Validated analyzed cohort: ",
  paste(names(observed_group_counts), as.integer(observed_group_counts),
        sep = "=", collapse = ", ")
)

# Supplementary table: SNV counts per clone before and after blacklist filtering
...
# =========================================================================
# STEP 35 — Per-clone SBS96 matrix construction and normalization
# =========================================================================

mut_mat <- mut_matrix(vcf_list = grl, ref_genome = CFG$ref_genome)
stopifnot(nrow(mut_mat) == 96L)
clone_totals <- colSums(mut_mat)
if (any(clone_totals == 0L)) {
  stop("At least one clone has zero filtered SNVs and cannot be normalized.")
}
mut_prop <- sweep(mut_mat, 2, clone_totals, "/")

# =========================================================================
# STEP 36 — Treatment-level SBS96 spectrum calculation
# =========================================================================
...
# =========================================================================
# STEP 37 — SIGNAL and COSMIC reference-signature loading
# =========================================================================
...
stopifnot(identical(rownames(mut_mat), rownames(signal_exp)),
          ...
          ncol(signal_exp) == 54L, ncol(cosmic_sbs) == 60L)
message(sprintf(
  "Loaded %d SIGNAL exposure signatures and %d COSMIC SBS signatures.",
  ncol(signal_exp), ncol(cosmic_sbs)
))

# =========================================================================
# STEP 38 — Prespecified reference resolution and cosine similarity
# =========================================================================
...
# =========================================================================
# STEP 39 — Table and figure generation
# =========================================================================

# Save Figure 2 panels in PDF, PNG, and SVG formats
...
# Figure S1
# Display-only thresholds: all values are represented by tile color; values
# >= 0.60 are printed numerically, and white text is used on darker tiles.
FIGS1_LABEL_THRESHOLD <- 0.60
FIGS1_WHITE_TEXT_THRESHOLD <- 0.78
...
build <- function(cm, xcols, dndband, dnd_dir) {
  ...
  tl$lab <- ifelse(tl$v >= FIGS1_LABEL_THRESHOLD, sprintf("%.2f", tl$v), "")
  tl$tcol <- ifelse(tl$v >= FIGS1_WHITE_TEXT_THRESHOLD, "white", "grey15")
...
message("Analysis complete. Outputs written to: ", normalizePath(CFG$outdir))
