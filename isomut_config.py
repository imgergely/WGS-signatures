...
Run with Python 2.7 from the IsoMut package directory after editing the path
constants as required. Table_S1_clone_key.csv defines the exact analysed panel.
EXCLUDED_SAMPLE_IDS is study-specific and must exactly match the original clone
IDs marked as omitted in Table S1.
"""
...
# ---------------------------- settings to edit ----------------------------
# Path to the IsoMut package directory containing src/.
ISOMUT_DIR = "."
...
# Every ID listed here must be marked as omitted in Table S1. Excluded BAMs
# may remain in INPUT_DIR for QC; only the 42 analysed BAMs are passed to IsoMut.
...
def load_panel(clone_key_path):
    ...
    if len(omitted_ids) != len(set(omitted_ids)):
        fail("Table S1 contains duplicated omitted clone IDs.")
    if set(analysed_ids) & set(omitted_ids):
        fail("A clone ID cannot be both analysed and omitted in Table S1.")
...
def has_bam_index(bam_path):
    ...
    root, _ = os.path.splitext(bam_path)
